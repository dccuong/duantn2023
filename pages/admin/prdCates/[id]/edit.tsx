...
const EditBlogCate: NextPageWithLayout = (props: Props) => {
  ...
  useEffect(() => {
    (async () => {
      try {
        const blogCate = await dispatch(getPrdCate(id)).unwrap();
        console.log("sssssssssssss", blogCate);
        reset(blogCate.cateproduct);
        setPreview(blogCate.cateproduct.name);
      } catch (error) {
        console.log(error);
      }
    })();
...
EditBlogCate.getLayout = (page: ReactElement) => (
